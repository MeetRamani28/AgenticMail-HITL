...
class GmailService:
    """
    Production-ready Gmail API Service for AgenticMail-HITL (Jarvis Edition).
    Handles OAuth 2.0 authentication, email fetching with full body parsing,
    real mailbox draft creation, and sending modern HTML/Text emails.
    """
    ...
    def create_draft(self, to_email: str, subject: str, body_content: str, thread_id: str = None, is_html: bool = True) -> str:
        ...
        try:
            ...
            print(f"📌 [GmailService] Draft created successfully in Gmail mailbox with ID: {draft.get('id')}")
            return draft.get("id")
        except Exception as e:
